...
int		teleporte_step_back(t_room *prev_room, t_room *room, t_queue *queue)
{
	...
	int_vector_copy(&room->prev_on_path->recipe.used_old_paths,
									&prev_room->recipe.used_old_paths);
	if (int_vector_push_back(&room->prev_on_path->recipe.used_old_paths,
														room->path_index) == 1)
		error_handler("Allocation error in teleporte??", NULL);
	void_vector_reset(&room->prev_on_path->recipe.start_old_path_room);
	void_vector_copy(&room->prev_on_path->recipe.start_old_path_room,
										&prev_room->recipe.start_old_path_room);
	void_vector_push_back(&room->prev_on_path->recipe.start_old_path_room,
																prev_room);
	void_vector_push_back(&room->prev_on_path->recipe.start_old_path_room,
																	room);
...
int		make_recipe(t_room *prev_room, t_room *room, t_queue *queue)
{
	...
	int_vector_reset(&room->recipe.used_old_paths);
	int_vector_copy(&room->recipe.used_old_paths,
								&prev_room->recipe.used_old_paths);
	void_vector_reset(&room->recipe.start_old_path_room);
	void_vector_copy(&room->recipe.start_old_path_room,
								&prev_room->recipe.start_old_path_room);
