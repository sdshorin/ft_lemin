...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/28 19:32:28 by bjesse            #+#    #+#             */
/*   Updated: 2019/11/28 19:32:29 by bjesse           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_room(t_room *now_room, t_queue *queue)
{
	...
	while (i < now_room->links.size)
	{
		if (now_room->path_index > -1
				&& ((t_room*)v_vector[i]) == now_room->next_on_path)
			;
		else if (now_room->path_index > -1
				&& ((t_room*)v_vector[i]) == now_room->prev_on_path)
...
int		find_new_way(t_data *data)
{
	...
	while (queue.start)
	{
		...
		if ((now_room->recipe.path_cost > data->max_path_cost
						&& data->max_path_cost >= 0) || now_room == data->start)
			continue ;
	...
	if (data->end->recipe.path_cost < 1 ||
			(data->end->recipe.path_cost > data->max_path_cost
